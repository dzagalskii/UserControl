...
_NetLocalGroupEnumT		     NetLocalGroupEnumPtr;
...
//загрузка функций из динамических библиотек
void Load_Dll(void)
{
    hNetapi32Dll = LoadLibrary(TEXT("netapi32.dll"));
    hAdvapi32Dll = LoadLibrary(TEXT("Advapi32.dll")); assert(hNetapi32Dll);
    //netapi32.dll
    ...
    NetLocalGroupDelPtr = (_NetLocalGroupDelT)GetProcAddress(hNetapi32Dll, "NetLocalGroupDel");                                 assert(NetLocalGroupDelPtr);                                                                assert(hAdvapi32Dll);
    NetLocalGroupEnumPtr = (_NetLocalGroupEnumT)GetProcAddress(hNetapi32Dll, "NetLocalGroupEnum");
